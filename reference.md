...
Board 快捷键：`Ctrl+1` 画板、`Ctrl+2` 演示、`Ctrl+I` 交互锁、`Ctrl+M` 修改、`Ctrl+F` 沉浸、`Ctrl+Shift+F` 浏览器全屏、`Ctrl+H` 演示热区、按住 `Space` 临时拖动画布、`Esc` 逐层退出、`?` 快捷键帮助。普通快捷键不会在输入框、文本域、下拉框或可编辑内容中触发。

画板索引支持拖拽和关闭；拖拽位置只保留到当前页面会话结束。工具栏设置可开关索引，显示状态以项目名为键保存在 `localStorage`；存储不可用时默认显示且不影响离线运行。
