...
assert.equal(shortcutIdForEvent(key('1', { ctrlKey: true })), 'canvas')
assert.equal(shortcutIdForEvent(key('2', { ctrlKey: true })), 'demo')
assert.equal(shortcutIdForEvent(key('i', { ctrlKey: true })), 'interaction')
assert.equal(shortcutIdForEvent(key('M', { ctrlKey: true })), 'review')
assert.equal(shortcutIdForEvent(key('f', { ctrlKey: true })), 'immersive')
assert.equal(shortcutIdForEvent(key('F', { ctrlKey: true, shiftKey: true })), 'browser-fullscreen')
assert.equal(shortcutIdForEvent(key('h', { ctrlKey: true })), 'hotspots')
assert.equal(shortcutIdForEvent(key('?', { shiftKey: true })), 'help')
assert.equal(shortcutIdForEvent(key('Escape')), 'escape')
assert.equal(shortcutIdForEvent(key('i', { metaKey: true })), null)
assert.equal(shortcutIdForEvent(key('i', { ctrlKey: true, altKey: true })), null)
assert.equal(shortcutIdForEvent(key('i', { ctrlKey: true, repeat: true })), null)
...
assert.match(boardSource, /<BoardSettings/)
