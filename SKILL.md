...
生成的原型带「修改」模式：用户可单选或多选页面节点，添加文字 / 顺序 / 删除 / 自由修改意见；每条意见会在目标旁显示半透明黄色编号，点击编号可浮动查看意见，最后生成可编辑并可复制给 AI 的 Prompt。修改模式下按住空格可临时拖动画布。为了让 Prompt 中的 DOM 选择器和批注标记能稳定映射回 JSX，生成或修改业务源码时必须遵守：
